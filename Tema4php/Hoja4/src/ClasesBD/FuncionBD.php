    ...
    public static function getProductos($tipo): array
    {
        try {
            ...
            if ($tipo === "alimentacion") {
                $resultado = $dwes->query('SELECT productos.id, productos.nombre, productos.precio, productos.categoria_id, alimentacion.mes_caducidad, alimentacion.anio_caducidad, categorias.id AS categoria_id, categorias.nombre AS categoria_nombre
                    FROM productos
                ...
                while ($row = $resultado->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $producto = new Alimentacion($row['id'], $row['nombre'], $row['precio'], $row['mes_caducidad'], $row['anio_caducidad'], $categoria);
                    $productos[] = $producto;
                }

            } else if ($tipo === "electronica") {
                $resultado = $dwes->query('SELECT productos.id, productos.nombre, productos.precio, productos.categoria_id, electronica.plazo_garantia, categorias.id AS categoria_id, categorias.nombre AS categoria_nombre
                    FROM productos
                ...
                while ($row = $resultado->fetch(PDO::FETCH_ASSOC)) {
                    $categoria = new Categoria($row['categoria_id'], $row['categoria_nombre']);
                    $producto = new Electronica($row['id'], $row['nombre'], $row['precio'], $row['plazo_garantia'], $categoria);
                    $productos[] = $producto;
                }

            } else {
                $resultado = $dwes->query('SELECT productos.id, productos.nombre, productos.precio, productos.categoria_id,
