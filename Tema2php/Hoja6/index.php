...
// Probar el método buscar (primero un elemento que existe)
echo "<h3>Buscar elemento que existe (Boeing 747):</h3>";
$aeropuerto->buscar("Boeing 747");

// Probar el método buscar (elemento que no existe)
...
// Probar el método listarCompania (compañía que existe)
echo "<h3>Listar aviones de la compañía Iberia:</h3>";
$aeropuerto->listarCompania("Iberia");

// Probar el método listarCompania (compañía que no existe)
...
// Probar el método rotores (helicóptero con un número de rotores que existe)
echo "<h3>Helicópteros con 4 rotores:</h3>";
$aeropuerto->rotores(4);

// Probar el método rotores (número de rotores que no existe)
...
// Mostrar si el avión está volando
if ($avionDespegado && $avionDespegado->volando()) {
    echo "El avión está volando.<br>";
    $avionDespegado->mostrarInformacion();
} else {
    echo "El avión no pudo despegar.<br>";
}
...
// Mostrar si el helicóptero está volando
if ($helicopteroDespegado && $helicopteroDespegado->volando()) {
    echo "El helicóptero está volando.<br>";
    $helicopteroDespegado->mostrarInformacion();
} else {
    echo "El helicóptero no pudo despegar.<br>";
}
