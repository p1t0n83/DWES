    ...
    <div class="bg-gray-900 p-8 rounded-xl shadow-lg w-full max-w-md">
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="mb-4">
                ...
                <input type="email" name="email" id="email" required autofocus class="w-full rounded p-2 text-black" value="{{ old('email') }}">
                @error('email')
            ...
            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-800 text-white px-4 py-2 rounded font-semibold transition">Entrar</button>
        </form>
        <div class="mt-6 text-center">
            <a href="{{ route('register') }}" class="text-blue-400 hover:underline">¿No tienes cuenta? Regístrate</a>
        </div>
    </div>
</div>
@endsection
