    ...
    <div class="max-w-xl mx-auto bg-gray-900 rounded-xl shadow-lg p-8 mt-10 text-white">
        ...
        <form action="{{ route('productos.update',$producto->slug) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nombre" id="nombre" class="w-full rounded p-2 text-black" value="{{$producto->nombre }}" required ></label>
            </div>

            <div class="mb-4">
                <label for="descripcion" class="block mb-1 font-semibold">Descripción
                <textarea name="descripcion" id="descripcion" class="w-full rounded p-2 text-black" required>{{$producto->descripcion }}</textarea></label>
            </div>

            <div class="mb-4">
                <label for="precio" class="block mb-1 font-semibold">Precio (€)
                <input type="number" name="precio" id="precio" class="w-full rounded p-2 text-black" value="{{$producto->precio }}"  step="0.01" required ></label>
            </div>

            <div class="mb-4">
                <label for="stock" class="block mb-1 font-semibold">Stock
                <input type="number" name="stock" id="stock" class="w-full rounded p-2 text-black" value="{{$producto->stock }}"  required ></label>
            </div>
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-800 text-white px-4 py-2 rounded font-semibold transition">Editar Producto</button>
        </form>
    </div>
@endsection
